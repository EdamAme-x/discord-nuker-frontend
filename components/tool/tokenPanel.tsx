	...
	return (
		<div className="max-w-[370px] flex flex-col justify-center items-center">
			<Label className="text-xl font-bold w-full px-[5px] inline-flex justify-center items-center my-3">
				TokenPanel <Setting data={props.data} setData={props.setData}><IoSettingsSharp className="ml-auto" /></Setting>
			</Label>
			<MultiSelect
			...
			<DialogContent className="sm:max-w-[425px]">
				...
				<div className="grid gap-4 py-4">
					<Button className="inline-flex justify-center items-center" onClick={() => {
                        ...
                        const answer = prompt(`"${password}"を入力することで、Tokenは完全に削除されます。\n よろしければ入力して下さい。`) ?? "fail"
                        if (password !== answer) {
                            toast.error("文字列が違うか、入力されていません。")
                        }else {
                            toast.success("削除成功")
                        }
                    }}><MdDelete className="transform scale-150 mr-2" /> Clear All Token</Button>
				</div>
			</DialogContent>
		</Dialog>
	);
